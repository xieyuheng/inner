...
# set-theoretical type theory

正如 2005-a-gentle-introduction-to-semantic-subtyping.md
中所说的那样。

函数类型 `(-> A B)` 首先可以理解为笛卡尔积 `A x B` 的有限子集的集合，
因为所有能用计算机实现的函数，本质上都是有限函数。

为了理解 `(-> int-t int-t)` 和 `(-> string-t string-t)` 的交集不为空，
`(-> A B)` 应该被理解为全集 `U` 的笛卡尔积 `U x U` 的有限子集的集合，
并且满足如果参数在 `A` 中，那么返回值在 `B` 中。
