...
* to pretend
  ...
  - x -
    I do not want to pretend I really like what I learned from the textbooks,
    they are not natural, not concrete, they are not good art.
    While I can, and I think I have to, join in their game.
    Can I ?
    I am so puzzled now,
    not only by the game of math,
    but also the game of playing the game,
    the game of life itself.
    So many 'truthes' about the motivation of the game have been said,
    but without given any evidence,
    as if motivation is not part of the game,
    as if the game is not played by human,
    thus the whole play become not convincing.
...
* 前提
  ...
  - k -
    因爲 實現一個代數拓撲的語言 的動機本身
    就是以更好的方式描述這些構造

  - x -
    但是爲什麼我不能直接研究代數拓撲的研究對象本身
