...
* I
  ...
  (01 * 0) f : (1 (0 * 0) f (1 * 0) f)
  (1 * 01) f : (1 (1 * 0) f (1 * 1) f)
  (01 * 1) f : (1 (0 * 1) f (1 * 1) f)
  (0 * 01) f : (1 (0 * 0) f (0 * 1) f)

  (01 * 01) f : (2 (01 * 0) f (1 * 01) f
                   (01 * 1) rev f (0 * 01) rev f)
  #+end_src
...
*** f & g & the proof
    ...
    (def h ;; to proof (f g ~ id of bool-suspend)
      (lambda (-> [bool-suspend I] [bool-suspend])
        (extend-from
          (lambda  (-> [bool-suspend (0 I)] [bool-suspend])
            (-> [:a i0] [:a f g])
            (-> [:a i1] [:a])))
        (with (-> [(0 bool-suspend) %:a (1 i0 i1)]
                  [(1 :a i0 <> :a i1 <>)])
          (-> [n i01]
              [(2 n refl)]
              : [(1 n n)])
          (-> [s i01]
              [#t m]
              : [(1 n s)]))
        (with (-> [(1 n s) %:b (1 i0 i1) %:i]
                  [(2 :b i0 <> s :i <>
                      :b i1 <> rev n :i <> rev)])
          (-> [#f m i01]
              [(3)]
              : [(2 (2 #f m #t m rev) #t m
                    #f m rev (2 n refl) rev)])
          (-> [#t m i01]
              [(3)]
              : [(2 (2 n refl) #t m
                    #t m rev (2 n refl) rev)]))))
